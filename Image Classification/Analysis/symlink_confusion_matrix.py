...
def create_confusion_matrix_structure(input_log, img_root, output_dir, task_index):
    ...
    for line in tqdm(lines, desc="Creating confusion matrix"):
        ...
        # CHANGED: Replaced .lexists() with a check for both .exists() and .is_symlink()
        if source_path.exists() and not (dest_path.exists() or dest_path.is_symlink()):
            try:
                # CHANGED: Replaced os.symlink with the pathlib .symlink_to() method
